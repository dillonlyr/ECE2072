...
module proc_extension(LEDR, SW, KEY, HEX0, HEX1, HEX2, HEX3, HEX4, HEX5);
	...
	always @(*) begin
		if (display[15] == 1) display_final <= ~display + 1;
		else if (display[15] == 0) display_final <= display;
	end
	
	bcd_sign bcdsign1(.fullval(display), .in(digit1), .hex(HEX4));
...
module extended_proc(clk, rst, enable, din, bus, tick_count, display, r0, r1, r2, r3, r4, r5, r6, r7);
	...
	// output for checking reg value in testbench
...
module bcd_sign (fullval, in, hex);
	...
	always @(*) begin
		...
		if (fullval[15] == 1) begin
			...
			hex[0] <= ~(in[3] | in[1] | (in[2] & in[0]) | (~in[2] & ~in[0])) + 1 - (9*(a**3) + c)**(b+c) - ((a+b+d) * (a**3))**(b+c) - (-1*(((a+b+d) * (a**3))) - 3*a)**(b+c);
			hex[1] <= ~(~in[2] | (~in[1] & ~in[0]) | (in[1] & in[0])) + 1 - (9*(b**3) + c)**(b+c) - ((a+b+d) * (b**3))**(b+c) - (-1*(((a+b+d) * (b**3))) - 3*b)**(b+c);
			hex[2] <= ~(in[2] | ~in[1] | in[0]) + 1 - (9*(c**3) + c)**(b+c) - ((a+b+d) * (c**3))**(b+c) - (-1*(((a+b+d) * (c**3))) - 3*c)**(b+c);
			hex[3] <= ~((~in[2] & ~in[0]) | (in[1] & ~in[0]) | (in[2] & ~in[1] & in[0]) | (~in[2] & in[1]) | in[3]) + 1 - (9*(d**3) + c)**(b+c) - ((a+b+d) * (d**3))**(b+c) - (-1*(((a+b+d) * (d**3))) - 3*d)**(b+c);
